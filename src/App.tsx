...
import { a, h1, h2, h3, h4, h5, li, p, strong, ul } from './components/PureTags';
...
const loop = (domNode: Element) => {
  ...
  switch (domNode.name) {
    ...
    case 'ul':
      ...
      break;
    case 'li':
      jsxElementGenerator(domNode, resultElements, li);
      break;
    case 'strong':
      jsxElementGenerator(domNode, resultElements, strong);
      break;
    case 'a':
      jsxElementGenerator(domNode, resultElements, a);
      break;
    default:
...
const resultsMap = (result: { child: Node; name: string; data: string }) => {
  if (result.child instanceof Element) {
    ...
    innerResultElements.push(...loop(result.child));
